 ...
 */
const getMissingItemReports = (): Promise<MissingItemReport[]> =>
  http.get<MissingItemReport[]>('lostandfound/missingitems');
 ...
 */
const getFoundItemReports = (): Promise<FoundItemReport[]> =>
  http.get<FoundItemReport[]>('lostandfound/founditems');
 ...
 * @returns the unique ID generated by the backend service for the report.
 */
const createFoundItemReport = (data: InitFoundItemReport): Promise<number> => {
  const now = new Date();
  const formattedData = {
    ...data,
    dateFound: data.dateFound || now.toISOString(), // Ensures dateFound is set
    dateCreated: now.toISOString(),
    colors: data.colors || [], // Ensures colors is an array, even if not defined
  };
  return http.post<number>('lostandfound/founditems', formattedData);
};

/**
 * Update an existing found item report.
 * @param data FoundItemReport object representing the updated report.
 * @param reportID The ID of the report to update.
 * @returns none
 */
const updateFoundItemReport = (data: FoundItemReport, reportID: number) => {
  const formattedData = {
    ...data,
    colors: data.colors || [], // Ensures colors is an array
  };
  return http.put(`lostandfound/founditems/${reportID}`, formattedData);
};
 ...
 * @returns none
 */
const updateFoundReportStatus = (id: number, status: string): Promise<void> =>

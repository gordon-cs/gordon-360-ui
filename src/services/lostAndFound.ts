...
const getMissingItemReports = (): Promise<MissingItemReport[]> =>
  http.get<MissingItemReport[]>('lostandfound/missingitemsall');
...
const getMissingItemReport = (id: number): Promise<MissingItemReport> =>
  http.get<MissingItemReport>(`lostandfound/missingitemsbyid/${id}`);
...
const getAdminActions = (id: number): Promise<MissingAdminAction[]> =>
  http.get<MissingAdminAction[]>(`lostandfound/missingitem/${id}/actionstakenall`);
 ...
 */
const createMissingItemReport = (data: Omit<MissingItemReport, 'recordID'>): Promise<number> => {
  const now = new Date();
  const formattedData = {
    ...data,
 ...
 */
const createAdminAction = (id: number, data: Omit<MissingAdminAction, 'id'>): Promise<number> =>
  http.post<number>(`lostandfound/missingitem/${id}/actiontaken`, data);
...
const updateMissingItemReport = (data: MissingItemReport, reportID: number) => {
  ...
  return http.put(`lostandfound/missingitem/${reportID}`, formattedData);
};
...
const updateReportStatus = (id: number, status: string): Promise<void> =>
  http.put<void>(`lostandfound/missingitem/${id}/${status}`);

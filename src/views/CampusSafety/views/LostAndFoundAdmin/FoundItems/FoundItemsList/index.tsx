import {
  ...
  // Decide the color based on how many days since date
  function dateAgeColor(isoString: string) {
    let dateGiven = Date.parse(isoString);
    let now = new Date();
    let dayDiff = differenceInCalendarDays(now, dateGiven);
  ...
  const FoundItemList = () => {
    ...
    const [reports, setReports] = useState<FoundItemReport[]>([]);
    ...
    useEffect(() => {
      const fetchInitial = async () => {
        try {
          ...
          const fetched = await lostAndFoundService.getFoundItemReports(
            tagID || '',
            status || '',
            color || '',
            category || '',
        ...
        <Grid container justifyContent="center" spacing={2} marginBottom={2}>
          <Grid item xs={12} md={11}>
            <Card className={styles.filterCardPosition}>
              <CardHeader
                title={<span className={styles.filterTitleText}><b>Filters: </b></span>}
          ...
          <AppBar className={styles.stickyHeader}>
            <Grid container className={styles.tableHeader} justifyContent="center">
              <Grid container item xs={11.85}>
                <Grid item xs={1.5}>Tag #</Grid>
                <Grid item xs={2}>Finder</Grid>
                <Grid item xs={2}>Location Found</Grid>
                <Grid item xs={1.5}>Category</Grid>
                <Grid item xs={3}>Description</Grid>
                <Grid item xs={1} className={styles.noWrap}>Date Found</Grid>
                <Grid item xs={1}></Grid>
              </Grid>
            </Grid>
          </AppBar>
                    ...
                    {reports.map((report) =>
                      isMobile ? (
                        ...
                        <Card
                          ...
                          onClick={() => navigate(`/lostandfound/lostandfoundadmin/founditemdatabase/${report.recordID}`)}
                        >
                          <CardContent>
                            ...
                            <Grid container justifyContent="space-between" alignItems="center">
                              ...
                              <Typography variant="body2" color={dateAgeColor(report.dateFound)}>
                        ...
                        <Grid
                          ...
                          onClick={() => navigate(`/lostandfound/founditems/${report.recordID}`)}
                        >
                          <Grid item xs={1.5}>
                            {report.recordID}
                          </Grid>
                          ...
                          <Grid item xs={1}>
                            <Typography color={dateAgeColor(report.dateFound)}>
                              {formatDate(report.dateFound)}
                            </Typography>
                          </Grid>
                          <Grid item xs={12}>
                            {statusChip(report)}

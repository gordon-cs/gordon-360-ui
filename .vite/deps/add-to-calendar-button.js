...
var atcbWcObjectParams = ["customLabels", "ty", "rsvp"];
var atcbWcObjectArrayParams = ["dates"];
var atcbWcArrayParams = ["images", "options"];
var atcbIcon = {
...
function atcb_date_specials_calculation(type, dateString, timeString = null, timeZone = null) {
  const tmpDate = function() {
    if (timeString) {
      return /* @__PURE__ */ new Date(dateString + " " + timeString);
    }
...
function atcb_check_required(data) {
  ...
  if (data.dates != null && data.dates.length > 0) {
    ...
    return requiredMultiField.every(function(field) {
      for (let i = 0; i < data.dates.length; i++) {
        if (!requiredMultiFieldFlex.includes(`${field}`) && (data.dates[`${i}`][`${field}`] == null || data.dates[`${i}`][`${field}`] == "") || requiredMultiFieldFlex.includes(`${field}`) && (data.dates[`${i}`][`${field}`] == null || data.dates[`${i}`][`${field}`] == "") && (data[`${field}`] == null || data[`${field}`] == "")) {
...
function atcb_validate(data) {
  ...
  if (!atcb_validate_icsFile(data, msgPrefix))
    return false;
  if (!atcb_validate_buttonStyle(data, msgPrefix))
    return false;
  if (!atcb_validate_subscribe(data, msgPrefix))
    return false;
  if (!atcb_validate_created(data, msgPrefix))
    return false;
  if (!atcb_validate_updated(data, msgPrefix))
    return false;
  if (!atcb_validate_options(data, msgPrefix))
    return false;
  if (!atcb_validate_date_blocks(data, msgPrefix))
    return false;
  if (!atcb_validate_rrule(data, msgPrefix))
    return false;
  if (data.recurrence_simplyfied) {
    if (!atcb_validate_rrule_simplyfied(data, msgPrefix))
      return false;
  }
  return true;
}
...
function atcb_validate_date_blocks(data, msgPrefix) {
  for (let i = 0; i < data.dates.length; i++) {
    ...
    if (!atcb_validate_icsFile(data, msgPrefix, i, msgSuffix))
      return false;
    if (!atcb_validate_status(data, msgPrefix, i, msgSuffix))
      return false;
    if (!atcb_validate_availability(data, msgPrefix, i, msgSuffix))
      return false;
    if (!atcb_validate_organizer(data, msgPrefix, i, msgSuffix))
      return false;
    if (!atcb_validate_attendee(data, msgPrefix, i, msgSuffix))
      return false;
    if (!atcb_validate_uid(data, msgPrefix, i, msgSuffix))
      return false;
    if (!atcb_validate_sequence(data, msgPrefix, i, msgSuffix))
      return false;
    if (!atcb_validate_timezone(data, msgPrefix, i, msgSuffix))
      return false;
    if (!atcb_validate_datetime(data, msgPrefix, i, msgSuffix))
      return false;
  }
  return true;
}
...
function atcb_validate_datetime(data, msgPrefix, i, msgSuffix) {
  ...
  if (!times.every(function(time) {
    if (data.dates[`${i}`][`${time}`] != null) {
      ...
      if (time == "startTime") {
        newDate.startDate = new Date(newDate.startDate.getTime() + timeParts[0] * 36e5 + timeParts[1] * 6e4);
      }
      if (time == "endTime") {
        newDate.endDate = new Date(newDate.endDate.getTime() + timeParts[0] * 36e5 + timeParts[1] * 6e4);
      }
    }
    return true;
  })) {
...
function atcb_close(host, keyboardTrigger = false) {
  ...
  if (allModals.length > 1) {
    ...
    Array.from(host.querySelectorAll(".atcb-list-wrapper")).concat(Array.from(host.querySelectorAll(".atcb-list"))).concat(Array.from(host.querySelectorAll("#add-to-calendar-button-reference"))).concat(Array.from(host.querySelectorAll("#atcb-bgoverlay"))).forEach((el) => el.remove());
    atcbStates["active"] = "";
  }
}
...
function atcb_generate_date_button(data, parent, subEvent = "all") {
  ...
  const fullTimeInfo = function() {
    ...
    if (timeZoneInfoStart == void 0 || timeZoneInfoStart == "" || formattedTimeStart.allday) {
      timeZoneInfoStart = "UTC";
    }
    if (timeZoneInfoEnd == void 0 || timeZoneInfoEnd == "" || formattedTimeEnd.allday) {
      timeZoneInfoEnd = "UTC";
    }
    let timeString = "";
...
var template = `<div class="atcb-initialized" style="display:none;position:relative;width:fit-content;"></div>`;
if (isBrowser()) {
...
function atcb_cleanup(host, data) {
  ...
  Array.from(host.querySelectorAll(".atcb-debug-error-msg")).concat(Array.from(host.querySelectorAll("style"))).concat(Array.from(host.querySelectorAll(".atcb-button-wrapper"))).forEach((el) => el.remove());
